...
export default function EmailVerificationPage() {
  ...
  }, [verificationStatus, dispatch, navigate, email, user_type]);

  return (
    <div
      className="flex flex-col items-center justify-center min-h-screen"
      style={{ backgroundImage: "url('/bgsign.jpg')" }}
    >
      <motion.div
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.6 }}
        className="bg-white p-3 rounded-full shadow-lg flex items-center justify-center w-20 h-20"
      >
        <FlightTakeoffIcon sx={{ fontSize: 40, color: "red" }} />
      </motion.div>
      ...
      >
        {status === "verifying" && (
          <>
            ...
            <p className="text-gray-500 mt-2">
              Completing the confirmation — please wait.
            </p>
          </>
        )}
        ...
        {status === "error" && (
          <>
            ...
            <p className="text-gray-500 mt-2">
              Invalid or expired link — try signing up again.
            </p>
          </>
        )}
      </motion.div>
