...
import { School, Balance, Language, FrontHand, CheckCircle } from '@mui/icons-material';
import { addCourseItem } from '../Redux/Slice/courseCartSlice';
...
const CoachingCards = () => {
  ...
  const courseCartItems = useSelector((state) => state.courseCart.items);
  
  // Track enrolled courses from cart state
  const enrolledCourseNames = new Set(courseCartItems.map((item) => item.course_name));

  useEffect(() => {
    const fetchCourses = async () => {
      const { data, error } = await supabase.from('visa_courses').select('*');
  ...
  const handleAddToCart = async (course) => {
    ...
    try {
      // Prepare cart item matching the schema
      // visa_courses has: course_name (text), price (text), description (text), img_url (text)
      // course_cart needs: course_name (text), price (numeric), description (text), img_url (text)
      const cartItem = {
        course_name: course.course_name, // Required
        description: course.description || null,
        img_url: course.img_url || null,
        price: course.price, // Will be converted to numeric in the thunk
      };

      console.log('Adding course to cart:', cartItem);

      const result = await dispatch(addCourseItem(cartItem));
      
      // The enrolled state will update automatically from Redux
      if (result.type === 'courseCart/addItem/rejected') {
        const errorMsg = result.payload || 'Failed to add course';
        console.error('Failed to add course:', errorMsg);
        // Don't show duplicate error - thunk already shows toast
        return;
      }
      
      // Success - enrolled state will update from Redux cart state
    } catch (error) {
                    ...
                    >
                      ...
                      <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', px: 2, pt: 2 }}>
                        ...
                        <Box sx={{ mt: 'auto', display: 'flex', justifyContent: 'center' }}>
                          {enrolledCourseNames.has(course.course_name) ? (
                            <MotionButton
                              variant="contained"
                              disabled
