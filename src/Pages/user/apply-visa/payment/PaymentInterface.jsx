...
export default function Payment({ onBack, onSuccess, user_id, countryWiseVisaDetails, application_id }) {
  ...
  const application_fees = Number(countryWiseVisaDetails?.find(visa => visa?.id == visaData?.visaId)?.visa_details[0]?.visa_fees);
  ...
  return (
    <div className="min-h-screen w-screen bg-white lg:h-screen lg:overflow-hidden">
      <div className="flex flex-col lg:h-full lg:grid lg:grid-cols-2">
        ...
        <OrderSummary onBack={onBack} allCharges={allCharges} visaData={visaData} visaSpecification={visaTypeSpecification?.visa_details[0]} application_fees={application_fees} total_amount={total_amount} />

        {/* Right Panel - Payment Form */}
        <PaymentForm personalInfoData={personalInfoData} allCharges={allCharges} visaData={visaData} visaSpecification={visaTypeSpecification?.visa_details[0]} application_fees={application_fees} total_amount={total_amount} application_id={application_id} country_id={country_id} passportData={passportData} />
